...
def pulp_fiction_actors
  ...
  Actor.select('DISTINCT actors.id, name').joins(:castings).joins(:movies).where("title = 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Actor.select('DISTINCT movies.id, title, yr').joins(:castings).joins(:movies).where("actors.name = 'Uma Thurman'")
